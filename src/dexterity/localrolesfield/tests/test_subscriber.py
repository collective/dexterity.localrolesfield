...
class TestSubscriber(unittest.TestCase):
    ...
    def test_object_modified(self):
        ...
        zope.event.notify(
            ObjectModifiedEvent(
                self.item,
                Attributes(ITestingBehavior, "ITestingBehavior.mono_localrole_field"),
                Attributes(ITestingType, "localrole_field", "localrole_user_field"),
            )
        )
        allowedRolesAndUsers = ctool.getIndexDataForUID("/".join(self.item.getPhysicalPath()))["allowedRolesAndUsers"]
        self.assertSetEqual(
            set(
                [
                    ...
                    "user:test_user_1_",
                    "user:kate",
                    u"user:support_editor",
                    u"user:support_reviewer",
                    u"user:jane",
                    u"user:tom",
                ]
            ),
    ...
    def test_object_modified_related(self):
        self.assertDictEqual(
            get_related_roles(self.folder, self.item.UID()),
            {
                u"mail_editor": set(["Editor"]),
                u"john": set(["Reviewer", "Reader"]),
        ...
        self.assertDictEqual(
            get_related_roles(self.folder, self.item.UID()),
            {
                u"support_editor": set(["Editor"]),
                u"jane": set(["Reader"]),
                u"tom": set(["Reader"]),
                u"basic-user": set(["Reviewer"]),
                u"kate": set(["Manager"]),
            },
        )
    ...
    def test_related_change_on_transition(self):
        ...
        self.assertDictEqual(
            get_related_roles(self.folder, self.item.UID()),
            {u"mail_editor": set(["Reader"]), u"john": set(["Editor"]), u"kate": set(["Editor"])},
        )
    ...
    def test_related_change_on_move(self):
        ...
        self.assertDictEqual(
            get_related_roles(folder1, self.item.UID()),
            {
                u"mail_editor": set(["Editor"]),
                u"john": set(["Reviewer", "Reader"]),
                u"kate": set(["Reader", "Manager"]),
    ...
    def test_local_role_configuration_updated(self):
        ...
        self.assertDictEqual(
            get_related_roles(self.folder, item1.UID()), {"jane": set(["Reader"]), "mail_editor": set(["Editor"])}
        )
        # Removing a state
        setattr(
            cls,
        ...
        self.assertDictEqual(
            get_related_roles(self.folder, self.item.UID()),
            {
                "kate": set(["Reader"]),
                "jane": set(["Reader"]),
                "mail_reviewer": set(["Reviewer"]),
                "mail_editor": set(["Editor"]),
            },
        )
        # Removing principal
        setattr(
        ...
        self.assertIn("user:mail_editor", allowedRolesAndUsers)
        self.assertDictEqual(
            get_related_roles(self.folder, self.item.UID()), {"jane": set(["Reader"]), "mail_editor": set(["Editor"])}
        )
